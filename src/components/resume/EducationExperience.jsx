const EducationExperience = [
    {
        id: 1,
        title: "Fullstack Web developer",
        company: "SoyHenry's bootcamp",
        yearsActive: "August 2022 - December 2022",
        information: [
            "Full stack developer with approximately 700+ hours of programming with techs such as Javascript, HTML, CSS Node, React, Redux,Git and SQL, by the way, I wanted to go a little further so I added some techs in the different projects that I did in this Bootcamp, techs as Typescript, PostgreSQL, Prisma, and NextJs. I worked with the agile methodology Scrum in my group work and libraries such as Auth0, MercadoPago, Cloudinary, etc.",
            "I did some Individual projects where apply techs as Node, Express, react, redux, Javascript and SQL.",
            "I did a group project where I had contributions in controller,routes, database models, cloudnary, payment gateway and login with auth0."
        ],
        certify:"https://certificates.soyhenry.com/cert?id=8f4a93c7-4c44-4eee-a2a8-ceaba7d39d64"
    },
    {
        id: 2,
        title: "Software development technician",
        company: "Jaime Isaza Cadavid Polytechnic",
        yearsActive: "May 2019 - December 2020",
        information: [
            "Here I started to development my logical Programmation skills, it was a hard process that I could to finish after two years and I got solid knowledge In HTML, CSS, Javascript, and react.",
            "I had a great experience at Polytechnic. I was able to meet new people from all walks of life, grow intellectually, and develop lifelong friendships. I was also able to explore different cultures and learn more about the world. Through classes, clubs, and extracurricular activities, I was able to broaden my knowledge and gain valuable skills that I still use today. I also had a lot of fun at Polytechnic, attending social events and participating in activities. It was a great experience that I will never forget.",
           
        ],
        certify:"https://drive.google.com/file/d/1anef62AlWUaEssEGjYfd9Yk6sKFM0qMN/view?usp=share_link"
    },
    {
        id: 3,
        title: "English B2",
        company: "Parlingo's academy",
        yearsActive: "October 2021 - December 2022",
        information: [
            "My experience at Academy English Parlingo has been great! The teachers are incredibly knowledgeable and passionate about the English language. They go out of their way to make sure that their students are learning in a fun and engaging way. The curriculum is well-structured, and the lessons are always interesting and enjoyable. I've learned a lot during my time here, and I'm confident that I'll be able to use the skills I've gained in the real world.",
            "I'm really proud of myself for getting a 90% average in Academy English Parlingo! It took a lot of hard work and dedication, but it was worth it. I started off by spending extra time on the material to make sure I understood the concepts. Then, I tried to practice as much as I could, using practice tests and quizzes to see what I knew and what I still needed to learn. Finally, I made sure to review my work after each assignment so that I could identify any mistakes and learn from them. All in all, I'm really glad I put in the effort and am proud of the outcome."
        ],
        certify:"https://drive.google.com/file/d/16QuWgHfzLv7FLO65GnkHeZSX_qJfwhw9/view?usp=share_link"
    },
    {
        id: 4,
        title: "English C1",
        company: "EF SET(International languages Campuses)",
        yearsActive: "December 2022",
        information: [
            "Getting a C1 in the EF SET English test requires a lot of effort. To get the highest possible score, you will need to have a thorough understanding of the English language, be able to communicate effectively in both spoken and written English, and be familiar with the test's format. You should also practice with sample tests to become familiar with the types of questions and the timing of the test. Additionally, you should review grammar and vocabulary rules to ensure you are able to accurately answer the questions. Finally, you should budget enough time to prepare for the test and practice to ensure you are ready for the actual test.",
        ],
        certify:"https://www.efset.org/cert/pckH5t"
    },

    {
        id: 5,
        title: "Javascrip University",
        company: "Udemy",
        yearsActive: "February 2022 - December 2022",
        information: [
            "I recently took a Javascript University course from Udemy. The course was taught by a professional programmer who had a lot of experience coding in Javascript. He was very knowledgeable and gave us a lot of helpful advice.",
            "The course was comprehensive, covering everything from the basics of the language to more advanced concepts. He used clear examples and provided us with plenty of practice exercises. We also had access to a community of other students who were taking the course. This was great for getting help and for discussing the course material.",
            "Overall, I found the Javascript University course from Udemy to be an incredibly valuable experience. It was well-structured, the instructor was knowledgeable and helpful, and I learned a lot. I would definitely recommend it to anyone interested in learning more about Javascript."
        ],
        certify:"https://udemy-certificate.s3.amazonaws.com/pdf/UC-25b84432-6e30-4712-b2b9-ec60151d59f4.pdf"
    },
];

export default EducationExperience;
